...
Sabemos que~$A$ é Monge convexa, pelo Teorema~\ref{Monge:keepConvex} todas as matrizes~$B_\ell$ são Monge convexas e podemos aplicar o SMAWK para encontrar seus máximos de linhas em tempo~$\Cl{O}(n)$. Já que o problema consiste em encontrar estes máximos para todas as~$k$ matrizes~$B_\ell$, conseguimos resolver o problema em tempo~$\Cl{O}(kn)$.
