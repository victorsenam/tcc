...
As matrizes Monge são objetos interessantes utilizadas em diversos contextos~\cite{Burkard}. O trabalho discute alguns algoritmos que aproveitam propriedades de matrizes Monge. Para introduzir tais matrizes e estes conceitos, olharemos para um problema interessante em geometria computacional.
\vskip2ex
Dado um polígono convexo~$p$ com os vértices indexados de 1 até~$n$ em sentido horário, encontrar, para cada vértice~$p_i$ com~$i \in [n]$, o vértice~$p_j$ com~$j \in [n]$ que maximiza a distância euclidiana~$d(p_i,p_j)$ entre os dois pontos.
\begin{figure}[h]
    \centering
...
\end{figure}
É fácil resolver o problema proposto de maneira em tempo~$\Cl{O}(n^2)$. Basta, para cada um dos~$n$ pontos, descobrir, em tempo~$\Cl{O}(n)$ qual é o outro ponto que maximiza a distância entre os dois. Existe alguma maneira mais rápida de resolver este problema? 
\vskip2ex
Uma primeira intuição falsa~\cite{Avis} que pode gerar soluções erradas para este problema é a ideia de que todo polígono convexo é unimodal. Um polígono unimodal é tal que para cada ponto a distância entre um ponto fixo~$p_i$ e os outros pontos percorridos em sentido horário possui apenas um máximo local. Uma solução melhor pode ser encontrada modelando este problema em termos de matrizes Monge.
\vskip2ex
Uma matriz~$A$ com~$n$ linhas e~$m$ colunas é \underline{Monge convexa} quando, para cada~$i,i' \in [n]$ e~$j,j' \in [m]$ onde~$i < i'$ e~$j < j'$, vale que
$$ A[i][j] + A[i'][j'] \leq A[i][j'] + A[i'][j] \text{.} $$
\vskip2ex
Vamos construir uma matriz Monge convexa~$A$ com~$n$ linhas e~$2n - 1$ colunas a partir de um polígono convexo~$p$ qualquer de~$n$ vértices de forma que, para cada linha~$i$, a coluna~$j$ que atinge o máximo nessa linha é tal que~$p_j$ é o ponto mais distante de~$p_i$. Se~$i < j < i+n$ vale que~$A[i][j] = d(p_i,p_j)$. Para~$j \leq i$ temos que~$A[i][j] = -1$ e, finalmente, para~$i + n \leq j$ vale que~$A[i][j] = j-i$.
\vskip2ex
...
Como encontrar o valor de máximo em cada linha de uma matriz Monge convexa sem analisar todas as suas entradas, isto é, mais rápido do que~$\Cl{O}(n^2)$? Para fazer isso precisamos explorar as propriedades das matrizes Monge. Toda matriz Monge convexa é \underline{monótona decrescente nos máximos das linhas}. Uma matriz leva este nome quando os índices de máximos das suas linhas é decrescente.
...
Mais do que monótona, uma matriz Monge convexa é \underline{totalmente monótona convexa nas linhas}, isto é, toda submatriz dela é monótona decrescente nos máximos das linhas. Estas propriedades são úteis para os algoritmos discutidos no trabalho.
\vskip2ex
...
\vskip2ex
Graças à total monotonicidade, é possível remover, em tempo~$\Cl{O}(n+m)$, linhas suficientes de forma que a matriz resultante ainda seja totalmente monótona e possua os máximos nas mesmas posições da original. O algoritmo todo toma tempo~$\Cl{O}(n+m)$.
...
A otimização de Knuth-Yao é uma aplicação das propriedades das matrizes Monge fortemente relacionada a programação dinâmica. Ela se baseia na solução de Knuth para o problema da árvore de busca binária ótima~\cite{Knuth}. Yao percebeu~\cite{Yao} que tal solução poderia ser usada para qualquer recorrência tal que~$A[i][j] = C[i][j]$ quando~$i = j$ e, quando,~$i < j$
\begin{equation*}
