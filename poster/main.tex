...
\usepackage[size=custom,height=84.1,width=59.4,scale=1.0]{beamerposter}
...
\vskip2ex
Graças à total monotonicidade, é possível remover colunas suficientes de forma que a matriz resultante ainda seja totalmente monótona e possua os máximos nas mesmas posições da original em tempo~${ \Cl{O}(n+m) }$. O algoritmo todo toma tempo~${ \Cl{O}(n+m) }$.
...
Tome um subvetor~$v[i \tdots j]$ de~$v$. Se~$\ell$ é um ponto ótimo para dividir~${ v[i \tdots j-1] }$ e~$r$ é um ponto ótimo para dividir~${v[i+1 \tdots j]}$, segundo Yao, um ponto ótimo~$p$ para dividir o vetor~$v[i \tdots j]$ deve respeitar~$\ell \leq p \leq r$.
